...
| **夜间静音** | 指定时间窗（支持跨天，如 23:00 → 07:00）内转速不超过夜间转速（取上限） |
| **温度守护** | 升到守护温度时转速不低于守护转速（取下限），降到退出温度才交还常规。带回差，避免在阈值附近抖动 |
| **起转回差** | 自动档起转后要低于「起转温度 − 回差」才停转，避免临界温度反复启停 |
...
```
温度守护（下限）  >  夜间静音（上限）  >  常规模式（auto / auto_curve / manual）
```

- **温度守护是「下限」不是「覆盖」**：最终转速 = `max(常规档位, guard_speed)`。
  常规档位本来就比守护转速高时沿用常规档位 —— 否则守护会把用户配的高温档压回去
  （典型：自动档配「90℃ → 80%」，守护却是「90℃ → 70%」，旧的覆盖写法会让 80% 永远生效不了）。
- **夜间静音是「上限」不是「固定值」**：最终转速 = `min(常规档位, night_speed)`。
  温度不高时该停还是停；温度真上去了就按常规档位走，夜间只负责「安静时别转太快」。
- **守护触发时夜间整体让位**：过热时散热优先于静音。
...
| `auto_temp_low/mid/high` | `70/80/90` | auto 模式三档温度阈值（低于起转温度停转） |
| `auto_pwm_low/mid/high` | `51/128/204` | auto 模式三档占空比，即 20% / 50% / 80% |
| `auto_hyst` | `3` | 起转回差（℃）。起转后要低于「起转温度 − 回差」才停，`0` = 到点即停 |
...
- **自动档默认改成 70 / 80 / 90℃**：70℃ 以下停转，70℃ 起转 20%，80℃ 50%，90℃ 80%（`auto_pwm` 默认 `51/128/204`）
- **修掉「高温档被温度守护压回去」的逻辑错误**：守护原来是**无条件覆盖**成 `guard_speed`，于是自动档配「90℃ → 80%(204)」时，守护（默认 90℃ / 70%(178)）会把 204 压回 178，用户配的 80% 永远生效不了。守护改为**取下限** `max(常规档位, guard_speed)`，只在常规档位比它低时兜底
- **夜间静音改为取上限** `min(常规档位, night_speed)`：原来是固定值，夜间 85℃（自动档要求 50%）风扇照样 0% 停转。现在温度不高该停还是停，温度真上去了按常规档位走；守护触发时夜间整体让位
- **自动档加起转回差**（`auto_hyst`，默认 3℃）：只有「起转 / 停转」这一档有回差，因为这一档是 0 ↔ 起转 PWM，温度在阈值附近抖动时风扇会反复启停。起转后要降到「起转温度 − 3℃」才停。判据直接用「当前输出转速 > 0」，不额外引入状态位，daemon 重启后依然正确。中温 / 全速两档只是升降档（风扇一直在转），不做回差
- **统一圆角与控件语言**：全页收成四档圆角变量 `--r-card:12px`（卡片）／`--r-box:10px`（盒子·按钮·凹槽外框）／`--r-ctl:8px`（槽内按键·输入控件）／`--r-pill:999px`（胶囊·开关·滑道）。原先散落的 6/8/9/10px 全部归位；主题偷偷塞给文本框的 4px 默认圆角也已压掉
- **PWM 滑条填充对齐滑块**：滑块的圆心行程是 `[拇指半径, 宽度-拇指半径]` 而不是 `[0, 宽度]`，以前直接用百分比当渐变断点，两端会错开半个滑块（中间刚好对上）。现在按 `th/2 + (100% - th) × p` 换算断点，0/25/50/75/100% 五点实测断裂点与滑块圆心完全重合
- **页面迁移到 ucode**：controller + 模板全部改为 ucode（`/usr/share/ucode/luci/`），LuCI 主题外壳照常（argon 侧边栏/顶栏）。模板经 `dispatcher` 的 template action 渲染，作用域自带 `include()`/`media`/`resource`/`dispatcher`，但**没有** `url()`——所有地址用 `dispatcher.build_url()` 或写死路径。Lua 版控制器与视图已随之删除，不再保留双实现
- **去掉按键/输入框的黑边**：argon 主题用 `button,select,input{border:1px solid #3c3c3c !important}` 给面板内所有按键和输入框强制套了近黑描边。已在 `#fc` 作用域内同优先级压掉，仅「保存/还原」按钮保留原本的浅灰 / 品牌蓝描边
